...
class Solution:
    def compareVersion(self, version1: str, version2: str) -> int:
        def making_arr(version,arr):
            i = j = 0
            while j < len(version):
                if version[j] != ".":
                    j += 1
                else:
                    arr.append(int(version[i:j]))
                    i = j + 1
                    j += 1
                if j == len(version):
                    arr.append(int(version[i:j]))
            return arr
        def adding_zeros(arr1,arr2):
            while True:
                arr2.append(0)
        ...
        arr1 = []
        arr2 = []
        arr1 = making_arr(version1,arr1)
        arr2 = making_arr(version2,arr2)
        if len(arr1) > len(arr2):
            adding_zeros(arr1,arr2)
        elif len(arr1) < len(arr2):
            adding_zeros(arr2,arr1)
        n = len(arr1)
        for i in range(n):
            if arr1[i] > arr2[i]:
                return 1
